...
export function ActivityFeed({ activities }: ActivityFeedProps) {
  if (activities.length === 0) {
    return (
      <p className="text-sm text-label italic py-4 text-center">
        No recent activity
      </p>
    );
  }
        ...
        return (
          <div key={activity.id} className="flex items-start gap-3">
            {activity.user.avatarUrl ? (
              ...
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-surface-elevated text-muted-foreground mt-0.5">
